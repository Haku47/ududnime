    ...
    <main class="container mx-auto px-4 py-12">
      <header class="mb-12 fade-in">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div class="flex items-center gap-4">
            ...
            <div>
              <h1 class="text-4xl md:text-5xl font-black uppercase tracking-tighter italic">
                EXPLORER <span class="text-[var(--accent-color)]">MODE</span>
              </h1>
              <p class="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] mt-1 italic">
                CARI BERDASARKAN GENRE, SEASON, ATAU TAHUN RILIS
              </p>
            </div>
          </div>
          ...
          >
            ...
            {{ loadingRandom ? 'MENCARI...' : 'RANDOM ANIME' }}
          </button>
        </div>
      </header>
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10 bg-slate-900/40 p-5 rounded-[2rem] border border-white/5 backdrop-blur-sm">
        <div class="flex items-center gap-3">
          ...
          <span class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Sort Results:</span>
        </div>
      ...
      <div class="flex flex-col lg:flex-row gap-10">
        <aside class="lg:w-64 space-y-6 fade-in h-fit lg:sticky lg:top-24">
          <div v-if="activeTab === 'genre'" class="bg-slate-900/40 p-6 rounded-[2.5rem] border border-white/5">
            <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 italic">GENRE LIST:</h3>
          ...
          <div v-if="activeTab === 'season'" class="bg-slate-900/40 p-6 rounded-[2.5rem] border border-white/5">
            <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 italic">SEASON LIST:</h3>
          ...
          <div v-if="activeTab === 'year'" class="bg-slate-900/40 p-6 rounded-[2.5rem] border border-white/5">
            <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 italic">YEAR RELEASE:</h3>
            <div class="flex flex-col gap-2 max-h-[400px] overflow-y-auto pr-2 custom-scroll">
              <button 
        ...
        <div class="flex-1">
          <div v-if="loading" class="flex flex-col justify-center items-center py-40">
            ...
            <p class="text-[10px] font-black uppercase text-slate-500 tracking-widest italic">LOADING JIKAN DATA...</p>
          </div>
          ...
          <div v-else>
            ...
            <div v-else class="text-center py-40 bg-slate-900/20 rounded-[3rem] border-4 border-dashed border-white/5">
              ...
              <p class="text-slate-600 text-[10px] font-black uppercase tracking-[0.3em]">TIDAK ADA HASIL GAIS.</p>
            </div>
          </div>
        </div>
      </div>
...
const handleToggleWatchlist = (animeData) => {
  ...
  if (idx === -1) {
    ...
    toastRef.value?.addToast("DITAMBAH KE LIST!", "success");
  } else {
...
const handleRemoveFromWatchlist = (id) => {
  ...
  let user = { ...currentUser.value };
  user.watchlist = user.watchlist.filter(i => i.id !== id);
  handleUpdateUser(user);
};
...
const fetchExplorer = async () => {
  ...
  try {
    ...
    toastRef.value?.addToast("GAGAL AMBIL DATA!", "error");
  } finally {
    loading.value = false;
  }
};
...
const fetchRandomAnime = async () => {
  ...
  try {
    ...
    toastRef.value?.addToast("GAGAL RANDOM!", "error");
  } finally {
